...
import {
  SaleOrUnsaleBlankeFetchCriteries,
  SaleOrUnsaleBlankeReport,
  Store,
...
@Component({
  selector: 'app-sale-or-unsale-blanke',
  templateUrl: './sale-or-unsale-blanke.component.html',
  styleUrls: ['./sale-or-unsale-blanke.component.css'],
})
export class SaleOrUnsaleBlankeComponent {
  store: Store[] = [];
  storeName: any;
  title: string;
  saleUnsale: SaleOrUnsaleBlankeReport[];
  totalCount: number = 0;
  ...
  fetchCriterias: SaleOrUnsaleBlankeFetchCriteries = Object.assign({});
  ...
  constructor(
    ...
    this.dataSource = new MatTableDataSource(this.saleUnsale);
  }
  ngOnInit() {
    this.title = 'Отчет по использованным и не использованным бланкам';
  ...
  onSearch() {
    ...
    this.fetchCriterias.isUsed = this.form.value.isUsed;
    ...
    this.reportService
      .getsaleUnSaleBlank(this.fetchCriterias)
      .subscribe((res) => {
